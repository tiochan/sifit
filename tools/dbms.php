 ...
 */
	...
	html_header($MESSAGES["DDBB_GENERIC_HEADER"]);
	...
	$dbms_type= get_http_param("p_dbms_type","");
	$host= get_http_param("p_host", "");
	$user= get_http_param("p_user", "");
	$pwd= get_http_param("p_pwd", "");
	$db_name= get_http_param("p_db", "");
	...
	<form method="POST">
		...
		<table class='data_box_rows'>
			...
			<tr style='background-color: #efffef;'>
				...
				<td valign="top" class='data_box_cell'>
					<table>
						<tr>
							<td>
								<b>DBType:</b>
							...
							<td>
								<select name='p_dbms_type'>
									<option value="mysqli" <?php if($dbms_type == "mysqli") echo "selected"; ?>>mysqli</option>
									<option value="oracle" <?php if($dbms_type == "oracle") echo "selected"; ?>>oracle</option>
									<option value="pgsql" <?php if($dbms_type == "pgsql") echo "selected"; ?>>pgsql</option>
								</select>
							</td>
						</tr>
						<tr><td><b>DBServer:</b></td><td><input name='p_host' value='<?php echo $host; ?>'></td></tr>
						<tr><td><b>DBName:</b></td><td><input name='p_db' value='<?php echo $db_name; ?>'></td></tr>
						<tr><td><b>DBUser:</b></td><td><input name='p_user' value='<?php echo $user; ?>'></td></tr>
						<tr><td><b>DBpasswd:</b></td><td><input type='password' name='p_pwd' value='<?php echo $pwd; ?>'></td></tr>
					</table>
...
<?php
	...
	if($query != "") {
		...
		require_once SYSHOME . "/include/dbms/" . $dbms_type . ".class.php";
		$dbtype= $dbms_type . "_class";
		$db= new $dbtype();

		$db->dbms_connect($host,$user,$pwd);

		if(!$db->dbms_select_db($db_name)) die("Error1!");

		if($num_rows) {
			$query= $db->dbms_limit($query, $num_rows, $offset);
		}
		...
		if($db->dbms_error() != "") {
			$ok="ERROR";
			html_showWarning("<b>ERROR EXECUTING QUERY:</b><hr><p><font color='red'>$query</font></p><hr><b>ERROR MESSAGE:</b><br>" . $db->dbms_error());
		} else {
			$ok="OK";

			if(@$db->dbms_check_result($res)) {
				@showRows($db,$res,true,true);
				$db->dbms_free_result($res);
			} else {
				$affected_rows= $db->dbms_affected_rows();
...
?>
	...
	<table class='data_box_rows' >
		...
		<tr>
			...
			<th width='30px' class='data_box_rows'>Id</th>
			<th width='40px' class='data_box_rows'>Result</th>

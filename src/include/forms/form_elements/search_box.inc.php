...
class search_box extends common_field_elements
{
	...
	public function get_restriction()
	{
		...
		if (($this->first_time) and count($this->values) == 0) {
			return "";
		}
		$add = "";
			...
			?>
				...
				<tr class='search_box_toolbar'>
					<td class='search_box_toolbar'>
						<input type='submit' class='action' value='<?php echo $MESSAGES["BUTTON_SEARCH"]; ?>' title='<?php echo $MESSAGES["BUTTON_SEARCH"]; ?>' onclick='document.forms.<?php echo $this->form_name; ?>.element.value="<?php echo $this->doc_name; ?>";
								   document.forms.<?php echo "$this->form_name." . $this->hidden_field_name; ?>.value="search";
								   document.forms.<?php echo $this->form_name; ?>.submit()'>
						<input type='reset' class='action' value='<?php echo $MESSAGES["BUTTON_RESET"]; ?>' title='<?php echo $MESSAGES["BUTTON_RESET"]; ?>'>
					</td>
				</tr>
			<?php
			}

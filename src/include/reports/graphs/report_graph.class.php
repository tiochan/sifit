...
abstract class report_graph
{
	 ...
	 * @param $labels -> array of strings/numbers ...
	 * @param $values -> array of arrays of values. Each array will be displayed as a new plot into the same graph.
	 * @param $plot_names -> For the legend.
	...
	protected function create_graph($graph_type, $filename = "")
	{
		...
		if (!$this->double) {
			...
			for ($i = 1; $i < $num_plots; $i++) {
				...
				if (!$is_bar) $this->graph->Add($lplots[$j]);
				else $bar_plots[] = &$lplots[$j];
			}

			// For bar plots, create accumulated or not...
			if (count($bar_plots) > 0) {
				if ($this->accumulated) {
					$gbbplot = new AccBarPlot($bar_plots);
					$group_plot = new GroupBarPlot(array($gbbplot));
				} else {
					$group_plot = new GroupBarPlot($bar_plots);
				}
				$this->graph->Add($group_plot);
			}


			// Now set parameters for each plot
			for ($i = 1; $i < $num_plots; $i++) {

				$j = $i - 1;

				$is_bar = (stripos($lplots_types[$i], "bar") !== false);

				$tmp_color_position = ($j % count($jpgraph_colors));
				//					$current_graph_color= $this->get_parameter("SERIE" . $i . "_COLOR", $jpgraph_colors[$tmp_color_position]);
				$current_graph_color = $this->get_parameter("SERIE" . $i . "_COLOR", false);
				...
				$current_graph_show_values = $current_graph_show_values === "true";

				//					$lplots[$j]->SetColor($current_graph_color);
				if ($current_graph_color) $lplots[$j]->SetColor($current_graph_color);
				...
				if ($this->show_values or $current_graph_show_values) {
					...
					if ($current_graph_color) $lplots[$j]->value->SetColor($current_graph_color);
					$lplots[$j]->value->SetFormat('%d');
					$lplots[$j]->value->Show();
				}

				if ($current_fill_color) $lplots[$j]->SetFillColor($current_fill_color . "@0.5");
			}
		} else {
